...
export default function Poll({
  ...
  const options = Array.isArray(poll?.options)
    ? poll.options.filter(Boolean)
    : [];
  const totalClicks = options.reduce(
  ...
  return (
    <div className="masonry-item sm:border-border sm:border sm:p-4 sm:rounded-md sm:!h-fit border-b pb-6">
      <Link
        className={cn("text-xl font-medium", titleClassName)}
        href={`/poll/${poll.id}`}
      >
        {poll?.title}
      </Link>
      ...
      <div className="grid gap-2 mt-5">
        {options
          ...
          .map((option, index) => {
            const percent =
              totalClicks > 0
                ? Math.floor((option.clicks / totalClicks) * 100)
                : 0;
            return (
              <Button
                disabled={loading || userVoteIndex === index}
                onClick={() => handlePollClick(poll.id, index)}
                variant="outline"
                key={index}
                className="disabled:opacity-80 flex gap-2 items-center justify-between relative z-10 bg-none after:rounded-md after:h-full after:absolute after:bottom-0 after:left-0 after:-z-10 after:bg-secondary after:w-[var(--vote-width)]"
                style={{
      ...
      <div className="flex items-center justify-between mt-3">
        <div className="flex items-center gap-2">
          ...
          >
            <Heart
              className={cn(
                "h-4 w-4",
                poll?.likes?.includes(user?.user?.id) && "fill-primary",
              )}
            />
            {poll?.likes?.length || 0}
          </Button>
        ...
        <div>
          <Button
            onClick={() => {
              handleShare(poll.title, poll.id);
            }}
