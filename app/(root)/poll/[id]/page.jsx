...
export default function Page({ params }) {
  ...
  const handlePollClick = async (pIndex) => {
    ...
    setLoading(true);
    try {
      const req = await updatePollResult({
        pollId: id,
        optionIndex: pIndex,
        userId: user.user.id,
      });
  ...
  const handleDeleteNestedComment = async (commentId, nestedCommentId) => {
    try {
      const req = await deleteNestedComment({
        pollId: id,
        parentCommentId: commentId,
        nestedCommentId,
        userId: user.user.id,
  ...
  const userVoteIndex = pollData?.clicks?.find(
    (click) => click.userId === user?.user?.id,
  )?.optionIndex;
  const totalClicks =
    pollData?.options
      ?.filter(Boolean)
  ...
  if (fetching) {
    return (
      <main className="py-10 mb-10 px-5 md:px-20 lg:px-32 md:grid grid-cols-2 gap-7 md:mt-10">
        <div>
    ...
    <main className="py-10 mb-10 px-5 md:px-20 lg:px-32 md:grid grid-cols-2 gap-7 md:mt-10">
      <div>
        ...
        <div className="grid gap-2 mt-4">
          {pollData?.options
            .filter((option) => Boolean(option.content))
            .map((option, index) => (
              <Button
                disabled={loading}
                ...
                style={{
                  "--vote-width": `${pollData?.options.filter(Boolean).reduce((acc, curr) => acc + curr.clicks, 0) > 0 ? ((option.clicks / pollData?.options.filter(Boolean).reduce((acc, curr) => acc + curr.clicks, 0)) * 100) | 0 : 0}%`,
                }}
              >
                <div className="flex items-center gap-2">
                ...
                {loading ? (
                  ...
                  <p className="text-sm opacity-80">
                    {totalClicks > 0
                      ? ((option.clicks / totalClicks) * 100) | 0
                      : 0}
                    %
                  </p>
                )}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Heart
                className={cn(
                  "h-4 w-4",
                  pollData?.likes.includes(user?.user?.id) && "fill-primary",
                )}
              />
        ...
        <div className="mt-3 grid gap-2">
          {comments
            ...
            .map((comment) => (
              <div
                key={comment.id}
                className="grid gap-2 border border-border p-3 rounded"
              >
                ...
                {comment.replies && comment.replies.length > 0 && (
                  <div className="mt-2 -mb-3">
                    {comment.replies.map((reply) => (
                      <div
                        key={reply.id}
                        className="mb-3 border-border border rounded-md p-2"
                      >
                        <div className="flex items-center justify-between">
                          ...
                          {user?.user?.id === reply.userId && (
                            <Button
                              ...
                              size="icon"
                              onClick={() => {
                                handleDeleteNestedComment(comment.id, reply.id);
                              }}
                              className="h-5 w-5"
                            >
                              <Trash className="h-2.5 w-2.5" />
